...
- CONFIG：启动时按 --env 选择 src/config/{env}.toml 加载（默认 dev），
  经 pydantic 校验后的只读配置。
- resources：进程级单例资源池。同步资源（应用元数据库、运行注册表）在
  import 时就绪；异步资源（LangGraph checkpointer 的 aiosqlite 连接）由
  FastAPI lifespan 阶段填充/关闭（见 utils/app_config.py）。
"""
import argparse
import sys
import tomllib
...
class Resources:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.db = AppDb(str(config.paths.data_dir / "app.db"))
        self.checkpointer = None  # AsyncSqliteSaver，由 lifespan 填充
        # sessionId -> 最近一次运行记录（services/runs.py），运行与页面连接解耦
        self.runs: dict = {}
