...
async def create_provider(body: ProviderBody, db: Session = Depends(get_db)):
    service.create_provider(db, body)  # 重名由主键约束触发 409（全局 handler）
    return api_ok()


@router.put("/providers/{name}")
async def update_provider(name: str, body: ProviderBody, db: Session = Depends(get_db)):
...
async def delete_provider(name: str, db: Session = Depends(get_db)):
