...
- **项目化会话管理**：侧栏按项目文件夹分组，新建会话时选择工作目录（最近使用一键选取），会话可重命名、可导出为 Markdown
- **本地执行**：agent 直接操作你机器上的文件和 shell（每个会话绑定一个工作目录）
- **项目记忆**：工作目录下的 `AGENTS.md`（或 `CLAUDE.md`）自动注入 system prompt（与 Claude Code 约定一致），agent 开箱即知项目规范
- **运行中追加消息**：任务执行途中可继续输入，消息在下一次模型调用前注入对话；运行结束仍未注入的自动作为新一轮发出
- **用量可见**：顶栏实时显示当前上下文占用与会话累计 tokens，刷新页面后从历史恢复
- **审批机制**：`execute` / `write_file` / `edit_file` / `delete` 等危险操作默认中断，界面上逐项批准或拒绝后继续；shell 命令可编辑后批准；勾选「总是允许」把命令前缀/工具加入项目审批白名单，命中的操作不再询问。审批状态落库，刷新页面甚至重启服务后仍在
...
**项目记忆**：每次运行构建 agent 时读取工作目录下的项目记忆文件拼进 system prompt，改完文件下一条消息即生效。只注入一个：优先 `AGENTS.md`，没有再找 `CLAUDE.md`（CLAUDE.md 通常引用 AGENTS.md，都注入会重复）；超长截断。

**运行中追加消息**：运行中输入框保持可用，发送的消息进入队列，在 agent 下一次调用模型前注入对话（正在执行的长命令/子代理要等当前步结束）；等审批时排队的消息在批准恢复后生效；运行收尾时仍未注入的自动作为新一轮发出。点「停止」会同时丢弃尚未注入的排队消息。

**token 用量**：顶栏「◔」chip 显示当前上下文占用（最近一次模型调用的输入+输出）与会话累计消耗，模型每次调用结束时刷新，重开页面从历史恢复。上下文压缩由 deepagents 内置的 SummarizationMiddleware 在超限时自动触发。

**模型服务**（设置 → 模型服务）：Cherry Studio 式配置。每个服务商有启用开关、API 地址、API 密钥（带「检测」按钮）和模型列表；顶栏模型 chip 按项目切换模型与参数（对项目下所有会话生效），不选则用全局默认。
...
审批卡片上每个操作独立决策：默认批准，可单独切换到拒绝（拒绝理由会反馈给模型）；shell 命令文本可直接编辑，按修改后的命令执行。勾选「本项目总是允许」把命令前缀（如 `git status`）或工具名加入项目审批白名单，之后命中的调用自动放行——复合命令（`&&`、`;`、管道拼接）要求每一段都命中白名单，含重定向或命令替换（`>`、`$()`、反引号）的命令永远照常审批。白名单保存在项目上，在顶栏模型菜单的「审批白名单」一节可逐条移除。

**导出会话**：顶栏「⬇ 导出」把当前会话完整历史（含思考过程、工具调用与结果）导出为 Markdown 文件。
